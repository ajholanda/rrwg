...
def save(rrwg):
    """Save the results of the simulation in a file.
    ...
    dataframe = np.full((nsteps, nverts*nwalks), 0.0, dtype=float)

    filename = 'd{}m{}n{}.dat'.format(nverts, nwalks, nsteps)
    for walk in range(nwalks):
        ysets = gen_ysets(rrwg, walk, nverts, nsteps)
        for i, row in enumerate(ysets):
            inf = walk*nverts
            sup = (walk+1)*nverts
            dataframe[i, inf:sup] = ysets[i]

    thefile = open(filename, 'w')
    for row in dataframe:
        for val in row:
            thefile.write('{:.2f} '.format(val))
        thefile.write('\n')
    thefile.close()
    print('* Wrote {}'.format(filename), file=sys.stderr)
